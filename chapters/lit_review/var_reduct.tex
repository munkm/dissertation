...
\begin{equation}
  P\big\{\bar{x} - \epsilon < \hat{x} \leq \bar{x} + \epsilon\big\} = \
   \int_{\bar{x}-\epsilon}^{\bar{x}+\epsilon}f_N\big( \hat{x} \big) d\hat{x}.
\end{equation}
This ends up as
\begin{equation}
  P\big\{\bar{x} - \epsilon < \hat{x} \leq \bar{x} + \epsilon\big\} = \
    \erf\Big[\sqrt{\frac{N}{2}} \frac{\epsilon}{\sigma(x)}\Big],
\end{equation}
and for large N this can be evaluated as
\begin{equation}
  P\big\{\bar{x} - M \sigma(x) < \hat{x} \leq \bar{x} + M \sigma(x) \big\} =
  \begin{cases}
    .683, & M = 1, \\
    .954, & M = 2, \\
    .997, & M = 3
...
calculate each mean. Further, as with all of the quantities in Section
...
$\hat{x}$ will not approximate $\bar{x}$.
