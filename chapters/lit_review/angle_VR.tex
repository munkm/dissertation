...
% RNS: what again? what is this epsilon? Do you mean \in?
...
Walters and Wareing \cite{walters_nonlinear_1994, walters_accurate_1996}
% RNS: Is that the right spelling of Wareing?
 showed
...
% RNS: How does that time comparison play in to the FOMs? unclear
...
significant. 
% RNS: I'm not sure that this is a point. No FOMs include user setup time... maybe just add the notion of the time burden genreally.
...
% RNS: again, you sometimes said it was better and sometimes worse, so 2 to 5 times here seems poorly supported.
...
Simple Angular CADIS \cite{peplow_consistent_2012} is based on
...
For the method with directionally-dependent weight windows and without
directional
...
% RNS: should the standard biased source also have a hat? Wait, is the 1/R * q *phi thing the original biased source? If so, then take out the phi here. 
...
% RNS: any comments about time for these two paragraphs? not sure how fom was impacted.
...
% RNS: I don't think you need that, or else provide a tripoli ref.
...
reconstruction.
The adjoint angular flux is approximated as piecewise continuous in space and
in angle
with Eqs.\ \eqref{eq:LIFTI} through \eqref{eq:LIFTIII}:
% RNS: I think here you want to do the subequation numbering to match the other sections. 
% Also, should all of the omegas have hats? And, should the scattering be g -> g'?
\label{LIFT}
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:LIFTIII}
\end{equation}
Turner showed that $\rho$
can be defined by deterministic scalar fluxes rather than from the cross section and $\lambda$, which requires some
assumptions on the distribution of particles:
% RNS: what are these assumptions? Either tell us or don't mention it. 
\begin{subequations}
\begin{equation}
...
% RNS: I'm not sure the point of this last sentence. I'd cut it (since you already told us) or contrast to other methods to make some point about it. 
...
but so did
AVATAR--confirming the findings of the previous study.
% RNS: What is different about this sentence than the one ending the previous paragraph? I added a clause to try to connect them.
 However, an important note was that a while
increasing the
accuracy of the deterministic solution may decrease the variance, it is not
necessarily the
best for the FOM. Thus, an
...
LIFT, flux at material interfaces is smoother and therefore the solution is both smoother and more physical.
Results were presented for a
1D, monoenergetic slab problem with material interfaces. The
modified version of LIFT outperformed both the original LIFT method and Monte Carlo
