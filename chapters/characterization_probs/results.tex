...
% a good variant would be another streaming material, like chrome
...
\end{table}
% the deleted comment should go in text, not caption.
...
% I wouldn't expect the omega method to be better here. I would expect it to just be slwoer. 
...
\end{figure}
% I don't know what your scaling statement means
% if you don't use a version where the legend is visible, either write what the numbers are or take the legend out and say the magnitude doesn't matter and the colors are for internal comparsion
...
With an intuition for how the $\Omega$- and adjoint scalar fluxes differ both on
...
% what values? what does the above sentence mean?
...
Based on the violin plots in Figure \ref{fig:beamviolins}, it was observed that
...
% don't understand that sentence either.
...
% what are the energy boundaries of that? Why is it in outlier in all 3 of them?!?
...
%While the U-shaped bend problem CADIS-$\Omega$ does not 
%significantly improve upon CADIS in terms of FOM, CADIS-$\Omega$ still achieved lower relative
%errors than CADIS. 
% already stated and unrelated to the rest of the paragraph.
...
% This is where I'd add some discussion about the translation of ray effects into the VR map. 
% that was one of the most important questions to answer, and this problem seems good for doing that. 
% I'd focus the analysis of this problem on that a little bit more. 
...
% I don't really understand what you're getting at in this sentence. Is it following particles that aren't useful just b/c they contain anisotropy? Why would it do that?
...
% I'd keep in mind in all problems, and especially here, it can be tricky to draw conclusions
% for some things b/c the errors are so high. You do address it, but might want to state it
% more strongly at the beginning of the section
...
% future: use an energy cutoff for this problem, no reason that the low energy groups should be used.
...
% do you leave out the other analysis b/c of the runtime issue?