...
factor of $r^2$ from the source. In a thin material, a particle may stream
...
very different moderating properties, such as the steel plate in
concrete, showed
...
used to characterize the $\Omega$-method's sensitivity to deterministic
parameter choice.
...
solution are the variables that are perturbed.
...
more effectively show the effect each parameter has on the results.
...
with the same relative error such that the trend as a result of changing
quadrature order is more observable. The first three sections of the table
pertain to different FOM values, and the last section of the table shows timing
results for the standard Monte Carlo and the total walltime for the entire
calculation.
...
at S$_N$ order 12 exist. These dips are much larger relatively than in the maximum or
...
CADIS-$\Omega$, conversely, has more varied results. No observable trend is
observable in the FOM with increasing quadrature order for CADIS-$\Omega$. A dip
...
takes CADIS-$\Omega$ to transport the standard Monte Carlo than CADIS. In the
...
much longer than CADIS', this impact is not as significant as expected.
Returning to Table
...
CADIS-$\Omega$ gets more particles to the tally region than CADIS. By increasing
quadrature order, CADIS-$\Omega$ generally increases the number of particles to
the tally as a whole, while CADIS does not. The table
...
average FOMs at intermediate quadrature orders.
...
In the characterization problems subsection, it was discussed that while the FOM
...
order, so the behavior of non-extreme tally bins can also be observed.
...
exist. Quadrature order 7 has relative errors much closer to quadrature orders
12 and above. Because tehse relative error spikes span so many bins, they affect
...
CADIS-$\Omega$, rather than CADIS. A number of interesting features exist in
...
similar relative error. This is not true in narrow energy bins, were higher
...
tally bin throws off the tally results, as observed in Table
...
indicate a lower relative error is achieved by CADIS. Returning again to the
...
CADIS. On this plot we see clearly that for higher energies, CADIS-$\Omega$
outperforms CADIS more in higher quadrature orders, as observed with I$_{RE}$.
...
P$_N$ order as they did for increasing quadrature order for CADIS-$\Omega$. In
Section \ref{subsec:quad_order} the idea that I/O requirements would severely
impact the FOM for CADIS-$\Omega$ was shown to be false. The FOM change between
FOM$_{MC}$ and FOM$_{hybrid}$ was roughly the same for CADIS as CADIS-$\Omega$
because the CADIS-$\Omega$ runtimes are so much longer than CADIS. Instead, the As
...
such as quadrature order 15, where both CADIS and CADIS-$\Omega$ has no spikes
...
CADIS' results in the same energy region just discussed show improvement in the
...
Figure \ref{fig:angle_fom_improvements} shows the
